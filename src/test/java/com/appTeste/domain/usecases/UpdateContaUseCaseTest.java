...
@ContextConfiguration(classes = {UpdateContaUseCase.class})
...
class UpdateContaUseCaseTest {
    ...
    /**
     * Method under test: {@link UpdateContaUseCase#execute(Conta)}
     */
    @Test
    void testExecute() {
        // Arrange
        ...
        when(contaRepository.update(Mockito.<Conta>any())).thenReturn(buildResult);
        Conta conta = new Conta();

        // Act
        updateContaUseCase.execute(conta);

        // Assert
        verify(contaRepository).update(isA(Conta.class));
